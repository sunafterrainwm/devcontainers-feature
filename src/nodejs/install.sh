...
OVERRIDE_NPM="${OVERRIDENPM:-false}"
...
npm_install() {
    ...
    if type $package > /dev/null 2>&1; then
        ...
        if type npm > /dev/null 2>&1; then
            ...
            echo "Skip installing $package because npm is missing"
        fi
    fi
}
...
# Corepack added in: Node.js v16.9.0, Node.js v14.19.0
HAVE_NATIVE_COREPACK=command -v corepack > /dev/null 2>&1 && true || false
if [ "${HAVE_NATIVE_COREPACK}" = "false" ]; then
    npm_install corepack
fi

if command -v corepack > /dev/null 2>&1; then
    if [ "${HAVE_NATIVE_COREPACK}" = "true" ] && [ "${OVERRIDE_NPM}" = "true" ]; then
        corepack enable npm yarn pnpm
    else
        corepack enable yarn pnpm
    fi
else
    echo "Skip installing yarn and pnpm because corepack is missing"
fi
